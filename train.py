...
# this is a similar generator, for validation data
    ...
    decay_rate = opt.lr / opt.epochs
    model=RecM.model_F
    #_adam=optimizers.SGD(lr=opt.lr, decay=decay_rate)
    model=RecM.model_F
    ...
    model_checkpoint = ModelCheckpoint(opt.chekp+'dp.hdf5', monitor='val_accuracy',verbose=1, save_best_only=True)
    #with tf.device('/device:GPU:0'):

    history=model.fit(
            train_generator,
            epochs=opt.epochs,
            validation_data=validation_generator,callbacks=[model_checkpoint,tensorboard_callback])
